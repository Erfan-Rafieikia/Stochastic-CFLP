...
class Callback:
    ...
    def __init__(self, dat: Data, y, eta):
        ...
        self.num_cuts_mip = {s: 0 for s in range(len(dat.S))}  # number of optimality cuts added for scenario s
        self.num_cuts_rel = {s: 0 for s in range(len(dat.S))}  # number of optimality cuts added (at node relaxation) for scenario s
    ...
    def __call__(self, mod, where):
        ...
        if where == GRB.Callback.MIPSOL:
            ...
            # Solve the subproblem and obtain its dual information for each scenario
            for s in self.dat.S:
                #Solve the correponding dual for subproblem of scenario s
        ...
        # Currently exploring a MIP node
            ...
            y_values = mod.cbGetNodeRel(self.y)  # Use relaxation values
